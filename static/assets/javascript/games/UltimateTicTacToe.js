...
function play_move(tboard, move, xturn) {
	...
	if (local_win(tboard, color, move, centerx - 1, centery - 1))
		tboard[centerx][centery] = color + 4;
	else if (square_full(tboard, centerx - 1, centery - 1))
...
function MCTS_simulate(father) {
	...
	while (!done) {
		...
		count = 0;
		if (next_center_color !== 5 && next_center_color !== 6 && next_center_color !== 3 && next_center_color !== 4) {
			// console.log("4", next_center_color);
			...
			// console.log(next_center[0], next_center[1], next_center_color, x, y);
			// console.log("3", x, y, tboard);
		}
		// if (next_center_color > 2)
		// 	console.log("!!! " + next_center_color);
			// do {
...
function choose_child(root) {
	if (!root.children)
		root.children = MCTS_get_children(root.State, root);
	if (root.children.length === 0) // leaf node
		...
		for (i = 0; i < root.children.length; i++)
			if (root.children[i].total_tries === 0)
				count_unexplored++;
		...
		if (count_unexplored > 0) {
			...
			for (i = 0; i < root.children.length; i++)
				if (root.children[i].total_tries === 0) {
					count_unexplored--;
					if (count_unexplored === 0) {
						run_simulation(root.children[i]);
						return;
					}
				}
			...
			var best_child = root.children[0], best_potential = MCTS_child_potential(root.children[0], root.total_tries), potential;
			for (i = 1; i < root.children.length; i++) {
				potential = MCTS_child_potential(root.children[i], root.total_tries);
				if (potential > best_potential) {
					best_potential = potential;
					best_child = root.children[i];
				}
			}
			choose_child(best_child);
		}
	}
}

function run_simulation(root) {
	back_propogate(root, MCTS_simulate(root));
}

function back_propogate(root, simulation) {
	if (simulation > 0)
		root.hits++;
	...
	root.total_tries++;
	if (root.parent)
		back_propogate(root.parent, -simulation);
}
